...
export const updateUser = async (req, res, next) => {
  try {
    ...
    const userCount = await UserModel.findOne({
      where: {
        DIDE: req.params.DIDE,
      },
    });
    ...
    userCount.password = hashedNewPassword;

    await userCount.save();

    return res.status(200).json({ msg: "Successfully Updated", userCount });
  } catch (error) {
    return next(error);
  }
};
...
export const addPatient = async (req, res, next) => {
  ...
  try {
    ...
    const addPatient = await UserModel.create({
      DIDE,
      userName,
      email,
    ...
    return res.status(200).json({ msg: 'Successfully Add Patient', addPatient });
  } catch (error) {
...
export const updatePassword = async (req, res, next) => {
  ...
  // Assuming req.doctor.id is the correct identifier
  ...
  const passwordMatch = bcrypt.compare(oldPassword, doctor.password);
  ...
  await doctor.save();
  doctor.changePasswordTime = Date.now();
...
export const getDoctor = async (req, res, next) => {
  try {
    const doctorId = req.DIDE;
    const doctor = await UserModel.findOne({ where: { DIDE: doctorId } });
    return res.status(200).json({ msg: "Successfully Show", "Data": doctor });
  } catch (error) {
...
export const updatePatient = async (req, res, next) => {
  try {
    ...
    if (!userCount) {
      throw new Error(`Invalid patient id `, { cause: 404 });
    }

    if (req.body.name) {
      userCount.userName = req.body.name;
    }

    const existingPatient = await PatientModel.findOne({ where: { email: req.body.email } });
    if (existingPatient) {
      throw new Error(`patient ${req.body.email} already exists`, { cause: 409 });
    }

    userCount.email = req.body.email;

    if (await bcrypt.compare(userCount.password, req.body.password)) {
      throw new Error(`password ${req.body.password} already exists`, { cause: 409 });
    }
    ...
    if (req.body.phoneNumber) {
      userCount.phoneNumber = req.body.phoneNumber;
    }

    await userCount.save();

    res.status(200).json({ msg: "Successfully Updating", userCount });
...
export const updateDoctor = async (req, res, next) => {
  try {
    ...
    await userCount.save();
    return res.status(200).json({ msg: "Successfully Updated", token: token, userCount });
  } catch (error) {
    return next(error);
  }
};
...
export const viewFile = async (req, res, next) => {
  try {
    const image = await ImageModel.findAll({ where: { DIDE: req.params.id } });
    if (image) {
      return res.status(200).json({ msg: "Successfully Show", "Data": image });
    }
    if (image == 0) {
      return res.status(200).json({ msg: "No Images for the Patient" });
    }
  } catch (error) {
    return res.status(500).json({ msg: "Failed to retrieve patient", error });
  }
}
//------------------------------------------------------------------------------
export const getTotalFile = async (req, res, next) => {
  try {
    const number = await ImageModel.count({
      where: { DIDE: req.params.id }
    });
    return res.status(200).json({ msg: "Successfully Count", "Data": number });
  } catch (error) {
    return res.status(500).json({ msg: "Failed to retrieve number", error });
  }
};
//----------------------------------------------------------------------------------
export const getPatient = async (req, res, next) => {
  try {
    const patient = await UserModel.findAll({ where: { role: "patient" } });
...
export const remove = async (req, res, next) => {
  try {
    let userData = await ResultModel.findOne({ where: { DIDE: req.params.id } });
    ...
    console.log("Error from deleting data", error.message);
    res.json({ msg: "Error in deleting data", errorMsg: error.message });
  }
};
//-------------------------------------------------------------------------------
export const getTotalPatient = async (req, res, next) => {
  try {
    const number = await UserModel.findAndCountAll();
    return res.status(200).json({ msg: "Successfully Count", "Data": number.count - 1 });
  } catch (error) {
    return res.status(500).json({ msg: "Failed to retrieve number", error });
  }
};
//-----------------------------------------------------------
export const getTotalImage = async (req, res, next) => {
  try {
    const number = await ImageModel.findAndCountAll();
    return res.status(200).json({ msg: "Successfully Count", "Data": number.count });
  } catch (error) {
    return res.status(500).json({ msg: "Failed to retrieve number", error });
  }
};
//------------------------------------------------------------------
export const getTotalResult = async (req, res, next) => {
  try {
    const number = await ResultModel.findAndCountAll();
    return res.status(200).json({ msg: "Successfully Count", "Data": number.count });
  } catch (error) {
    return res.status(500).json({ msg: "Failed to retrieve number", error });
  }
...
export const getSpecificPatient = async (req, res, next) => {
  ...
  const p = await UserModel.findOne({ where: { userName } });
  const patient = p.DIDE;
  res.json({ msg: "success", "Data": patient });
